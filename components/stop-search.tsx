...
export function StopSearch({
  ...
  const results = useMemo(() => searchStops(query), [query])
  ...
  return (
    <div className="relative w-full max-w-xl">
      <div
        className={`flex items-center gap-3 rounded-full border bg-background shadow-lg outline outline-2 outline-transparent transition-all duration-300 ease-out focus-within:shadow-xl focus-within:outline-ring/40 ${compact ? "px-4 py-2" : "px-5 py-3"}`}
      >
        <Search
          className={`shrink-0 text-muted-foreground ${compact ? "h-4 w-4" : "h-5 w-5"}`}
        />
        <input
          type="text"
          value={query}
      ...
      {showResults && (
        <div className="absolute top-full right-0 left-0 z-50 mt-2 animate-in overflow-hidden rounded-2xl border bg-popover shadow-xl duration-200 fade-in slide-in-from-top-2">
          {results.length === 0 ? (
