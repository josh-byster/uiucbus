...
import { getSavedStops, removeSavedStop, clearSavedStops } from "@/lib/saved-stops";
            ...
            <DropdownMenuContent align="end" className="w-80">
              {savedStops.length === 0 ? (
                <div className="flex flex-col items-center gap-1 px-4 py-6 text-center">
                  ...
                  <p className="mt-1 text-sm font-medium text-muted-foreground">No recent stops</p>
                  <p className="text-xs text-muted-foreground/60">Stops you visit will appear here</p>
                </div>
              ) : (
                <>
                  <div className="px-3 py-2">
                    <p className="text-xs font-medium text-muted-foreground">Recently visited</p>
                  </div>
                  {savedStops.map((stop) => (
                    <DropdownMenuItem
                      key={stop.id}
                    ...
                    >
                      ...
                      >
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="text-xs text-muted-foreground">{stop.id}</p>
                        </div>
                      </Link>
                      <button
                        onClick={(e) => {
                          ...
                          setSavedStops((prev) => prev.filter((s) => s.id !== stop.id));
                        }}
                        className="shrink-0 rounded-md p-1 text-muted-foreground/0 transition-colors hover:bg-destructive/10 hover:text-destructive group-hover:text-muted-foreground/50"
                      >
